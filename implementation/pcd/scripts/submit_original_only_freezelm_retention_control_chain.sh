...
# Prepared branch: use if low LR still fails.  This keeps the language backbone
# frozen and tests whether the retention collapse comes from updating the LLM
# weights rather than from the exported data loader or evaluation path.
...
ABLATION_NAME=${ABLATION_NAME:-origonly24k_sft_freezelm_retention}
...
export EXTRA_ARGS=${EXTRA_ARGS:-"num_train_epochs=1.0 freeze_language_model=true learning_rate=1.0e-5 save_steps=1000 save_total_limit=1 report_to=none"}
